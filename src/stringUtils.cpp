...
*/
std::wstring stripQuotes(const std::wstring& s) { // <-- 1. Am adăugat & pentru a se potrivi cu headerul
    std::wstring result = wstr_trim(s);           // <-- 2. Facem o copie locală pentru a o putea modifica

    if (result.size() >= 2) {
        wchar_t first = result.front();
        wchar_t last = result.back();

        // Curățăm ', ", ` și chiar paranteze drepte [] (stil SQL Server)
        if ((first == L'\'' && last == L'\'') ||
            (first == L'\"' && last == L'\"') ||
            (first == L'`' && last == L'`') ||
            (first == L'[' && last == L']')) {
